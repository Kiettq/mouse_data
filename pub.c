...
int open_device() {
    ...
    if (fd < 0) {
        if (errno == ENOENT || errno == ENODEV) {
            printf("⚠️  Thiết bị không tồn tại hoặc đã bị ngắt kết nối!\n");
        } else {
            perror("❌ Lỗi mở thiết bị");
        }
    } else {
        printf("✅ Đã kết nối lại với thiết bị!\n");
    }
    return fd;
}
...
int main() {
    ...
    while (1) {
        if (fd < 0) {
            // Gửi thông điệp disconnect MQTT
            const char *disconnect_msg = "{\"status\": \"device_disconnected\"}";
            mosquitto_publish(mosq, NULL, MQTT_TOPIC, strlen(disconnect_msg), disconnect_msg, 0, false);

            // Thử mở lại thiết bị mỗi 1 giây
            sleep(1);
            fd = open_device();
            continue;
        }
        ...
        if (bytes <= 0) {
            if (errno == ENODEV || errno == EIO) {
                printf("⚠️  Mất kết nối với thiết bị!\n");
                close(fd);
                fd = -1;
                continue;
            }
